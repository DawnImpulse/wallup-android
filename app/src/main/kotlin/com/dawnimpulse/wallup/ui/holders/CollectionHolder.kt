...
class CollectionHolder(view: View) : RecyclerView.ViewHolder(view) {
    ...
    private val point = F.displayDimensions(context)
    private val width = point.x / 2

    fun bind(item: WallupCollectionObject) {

        val height: Int = if (item.vertical)
            F.dpToPx(240, context)
        else
            F.dpToPx(180, context)

        layout.layoutParams = FrameLayout.LayoutParams(width, height)
        image.setParams(width - F.dpToPx(8, context), height - F.dpToPx(8, context))

        text.text = item.name

        if (item.images.isNotEmpty())
            ImageHandler.setImageImgix(image, item.images[0].urls[0], 480)
    }
}